...
void Lexer::load_config( const string &path )
{
    ...
    for (std::string line; std::getline(input1, line); j++)
    {
        ...
        if(constexpr unsigned int ID_PAIR = 1; token_match[ID_PAIR].matched)
        {
            ...
            _id_tab[tok.id] = tok;
            _idx_tab[tok.index] = tok;
            _name_tab[tok.name] = tok;
        }
    }
...
void Lexer::dump_config( ) const
{
    ...
    {
        token tok = _tokens[i];
        ss << "Id: " << left << setw(10) << tok.id <<
            " name: " << right << setw(15) << tok.name <<
            " type: " << left << setw(15) << tok.stype <<
            " value: " << left << setw(15) << tok.value <<
            " rexp: " << left << setw(15) << tok.rexp <<
            " test_value: " << left << setw(15) << tok.test_value << endl;
    }
    cout << ss.str();
...
void Lexer::get_expr( /*out*/ string& s )
{
    ...
    for(int i = 0; i < len; ++i)
    {
        boost::regex_match( _tokens[i].test_value, m, rgx );
        const size_t sz = m.size();
        for(int j = 0; j < sz; ++j)
        {
            if(m[j].matched)
            {
                cout << "Found match for " << _tokens[i].name << " syncing index. " << j << endl;
                _tokens[i].index = j;
                break;
            }
        }
